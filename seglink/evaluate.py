...
# testing
tf.app.flags.DEFINE_string(
    'image_resize_method', 'fixed', 'Image resizing method. "fixed" or "dynamic"')
...
tf.app.flags.DEFINE_float('node_threshold', 0.5,
                          'Confidence threshold for nodes')
tf.app.flags.DEFINE_float('link_threshold', 0.5,
...
tf.app.flags.DEFINE_string(
    'result_format', 'icdar_2015_inc', 'Result file format')
tf.app.flags.DEFINE_string('result_suffix', time.strftime(
    '_%Y%m%d_%H%M%S'), 'Result file suffix')
# post processing
...
tf.app.flags.DEFINE_integer('load_intermediate', 0,
                            'Whether to load intermediate results.')
tf.app.flags.DEFINE_integer('save_intermediate', 0,
                            'Whether to load intermediate results.')
    ...
    with tf.Session(config=sess_config) as sess:
        ...
        if FLAGS.load_intermediate:
            ...
            if FLAGS.save_intermediate:
                ...
                logging.info('Intermediate result saved to {}'.format(
                    intermediate_result_path))
        ...
        if FLAGS.result_format == 'icdar_2015_inc':
            ...
            logging.critical(
                'Unknown result format: {}'.format(FLAGS.result_format))
            sys.exit(1)
...
def postprocess_and_write_results_ic15(all_batches, result_dir):
    ...
    for batch in all_batches:
        for i in range(FLAGS.test_batch_size):
            ...
            polygons[:, ::2] = np.maximum(0, np.minimum(
                polygons[:, ::2] * scale_x, orig_w - 1))
            polygons[:, 1::2] = np.maximum(0, np.minimum(
                polygons[:, 1::2] * scale_y, orig_h - 1))
            ...
            with open(save_path, 'w') as f:
                ...
                f.write('\r\n'.join(lines))
                logging.info(
                    'Detection results written to {}'.format(save_path))
...
def postprocess_and_write_results_ic13(all_results):
    ...
    for j in range(batch_size):
        ...
        scale_y = float(orig_h) / float(resize_h)
        scale_x = float(orig_w) / float(resize_w)
        bboxes[:, ::2] = np.maximum(0, np.minimum(
            bboxes[:, ::2] * scale_x, orig_w - 1))
        bboxes[:, 1::2] = np.maximum(0, np.minimum(
            bboxes[:, 1::2] * scale_y, orig_h - 1))
