...
tf.app.flags.DEFINE_string(
    'log_dir', '', 'Directory for saving checkpoints and log files')
tf.app.flags.DEFINE_string('log_prefix', '', 'Log file name prefix')
# training
tf.app.flags.DEFINE_string(
    'resume', 'vgg16', 'Training from loading VGG16 parameters ("vgg16"), resume a checkpoint ("resume"), or finetune a pretrained model ("finetune")')
tf.app.flags.DEFINE_string(
    'vgg16_model', '../model/VGG_ILSVRC_16_layers_ssd.ckpt', 'The pretrained VGG16 model checkpoint')
tf.app.flags.DEFINE_string('finetune_model', '', 'Finetuning model path')
tf.app.flags.DEFINE_string(
    'train_datasets', '', 'Training datasets file names separated by semicolons')
tf.app.flags.DEFINE_string(
    'weight_init_method', 'xavier', 'Weight initialization method')
...
tf.app.flags.DEFINE_float('hard_neg_ratio', 3.0,
                          'Ratio of hard negatives to positives')
tf.app.flags.DEFINE_integer(
    'no_random_crop', 0, 'In data augmentation, do not crop image, i.e. use full images')
# optimizer
...
tf.app.flags.DEFINE_integer(
    'max_steps', 60000, 'Maximum number of iterations.')
tf.app.flags.DEFINE_string('lr_policy', 'staircase',
                           'Learning rate decaying policy')
tf.app.flags.DEFINE_string(
    'lr_breakpoints', '', 'Comma-separated breakpoints of learning rate decay')
tf.app.flags.DEFINE_string(
    'lr_decays', '', 'Comma-separated decay values for every breakpoint')
tf.app.flags.DEFINE_integer(
    'profiling', 0, 'Do profiling during training (profiling could slow down training significantly)')
tf.app.flags.DEFINE_integer(
    'profiling_step', 21, 'Run profiling once at this step')
tf.app.flags.DEFINE_string(
    'profiling_report', 'timeline.json', 'Profiling report filename')
# summaries and checkpoints
tf.app.flags.DEFINE_integer('brief_summary_period',
                            10, 'Period for brief summaries')
tf.app.flags.DEFINE_integer(
    'detailed_summary_period', 200, 'Period for detailed summaries')
tf.app.flags.DEFINE_integer(
    'checkpoint_period', 5000, 'Period for saving checkpoints')
        ...
        with tf.device('/cpu:0'):
            ...
            with tf.name_scope('lr_decay'):
                if FLAGS.lr_policy == 'staircase':
                    ...
                    lr_breakpoints = [int(o)
                                      for o in FLAGS.lr_breakpoints.split(',')]
                    ...
                    for lr_decay, lr_breakpoint in zip(lr_decays, lr_breakpoints):
                        fn = (lambda o: lambda: tf.constant(
                            o, tf.float32))(lr_decay)
                        pred_fn_pairs.append(
                            (tf.less(self.global_step, lr_breakpoint), fn))
                    lr_decay = tf.case(pred_fn_pairs, default=(
                        lambda: tf.constant(1.0)))
                else:
                    logging.error(
                        'Unkonw lr_policy: {}'.format(FLAGS.lr_policy))
                    sys.exit(1)
            ...
            with tf.name_scope('input_data'):
                ...
                for i, dataset in enumerate(train_datasets):
                    if not os.path.exists(dataset):
                        logging.critical(
                            'Could not find dataset {}'.format(dataset))
                        sys.exit(1)
                    logging.info(
                        'Added training dataset #{}: {}'.format(i, dataset))
                ...
                logging.info('Batch size {}; capacity: {}; min_after_dequeue: {}'.format(
                ...
                logging.info('Batch size is {} on each of the {} GPUs'.format(
                    sub_batch_size, FLAGS.n_gpu))
                sub_batches = []
                for i in range(FLAGS.n_gpu):
                    sub_batch = {}
                    for k, v in train_batch.items():
                        sub_batch[k] = v[i *
                                         sub_batch_size: (i + 1) * sub_batch_size]
                    sub_batches.append(sub_batch)

            if FLAGS.optimizer == 'sgd':
                optimizer = tf.train.MomentumOptimizer(
                    self.current_lr, FLAGS.momentum)
                logging.info(
                    'Using SGD optimizer. Momentum={}'.format(FLAGS.momentum))
                ...
                logging.critical(
                    'Unsupported optimizer {}'.format(FLAGS.optimizer))
                sys.exit(1)
            ...
            for var in tf.global_variables():  # tf.all_variables():
                ...
                tf.summary.histogram(
                    summary_name, var, collections=['detailed'])
        ...
        with tf.Session(config=sess_config) as sess:
            ...
            if FLAGS.resume == 'resume':
                ...
                if latest_ckpt_path is None:
                    logging.error(
                        'Failed to find the latest checkpoint from {}'.format(FLAGS.log_dir))
                    sys.exit(1)
                else:
                    model_loader = tf.train.Saver()
                    model_loader.restore(sess, latest_ckpt_path)
                    logging.info('Resuming checkpoint %s' % latest_ckpt_path)
            # It just like the above 'resume'. The only difference is that this is a complete model.
                ...
                vgg16_vars = tf.get_collection(
                    tf.GraphKeys.GLOBAL_VARIABLES, scope='ssd/vgg16/')
                pretrained_loader = tf.train.Saver(var_list=vgg16_vars)
                pretrained_loader.restore(sess, FLAGS.vgg16_model)
                logging.info(
                    'VGG16 parameters loaded from {}'.format(FLAGS.vgg16_model))
            ...
            with slim.queues.QueueRunners(sess):
                ...
                for step in tqdm_range:
                    ...
                    if need_profiling:
                        ...
                        tl_write_path = os.path.join(
                            FLAGS.log_dir, FLAGS.profiling_report)
                        with open(tl_write_path, 'w') as f:
                            f.write(ctf)
                        logging.info(
                            'Profiling results written to {}'.format(tl_write_path))
                        # write to summary
                        summary_writer.add_run_metadata(
                            run_metadata, 'step_{}'.format(step))
